...
const PaginaEditar = () => {
    ...
    fetch(`http://localhost:5000/teste/${id}`)
      .then(response => {
        response.json().then(teste => {
          setTesteInfo(teste)
          setTipo(teste.tipo);
          setAno(teste.ano_criacao);
          setConteudo(teste.conteudo);
          setDocente(teste.docente);
          setAnoSelecionado(teste.ano);
          setDisciplinaSelecionada(teste.cadeira);
        })
      })
  }, []);


  async function atualizarTeste(ev) {
  ...
  return (
    <div className="box">
      <div className="title">
        <h2 style={{ color: "#4FC3A1" }}>Editor de Testes e Exames</h2>
      </div>
      <form onSubmit={atualizarTeste}>
        ...
        <div className="form-linha">
          ...
          {testeInfo && (
            <div className="form-grupo">
              <label for="semestre">Semestre:({testeInfo.semestre}) </label>
              <select
        ...
        <button className="button">Atualizar Teste</button>
      </form>
    </div>
  );
